# Аудит текущей методологии

Дата: 2026-07-14

Область: инвентарь и классификация текущих модулей Intensive English, lesson flows, exercise types, repetition mechanics, AI interactions, assessments, content sources и user flows.

Классификация: KEEP, KEEP BUT REPOSITION, REDESIGN, REPLACE, REMOVE, RESEARCH FURTHER.

## 1. Короткий вердикт

В текущем Intensive English уже есть правильная амбиция: это не должно быть ещё одно quiz app. В системе уже есть сильный input engine, meaning support на родном языке, productive SRS, speech output, role scenes, shadowing, reading, grammar frames, feedback prompts и psychology layer.

Проблема - в последовательности и доказательности:

- recognition всё ещё слишком близко к “learned”;
- output есть, но ему нужна более строгая ladder;
- grammar promising, но должна быть diagnostic и transfer-oriented;
- community концептуально ценна, но пока under-specified;
- intensive claims нуждаются в evidence boundaries;
- mobile и web всё ещё частично копии, а не method-specific environments.

## 2. Inventory Matrix

| Item | Skill/subskill | Learner problem | Mechanism | Evidence | Assumes | Mode trained | Transfer? | Placement | Platform | Decision |
|---|---|---|---|---|---|---|---|---|---|---|
| Onboarding: родной язык | доступ к смыслу | translation mismatch | L1 support | Research-supported | learner knows L1 | diagnostic | indirect | before training | both | KEEP |
| Onboarding: interface language | понимание интерфейса | app speaks wrong language | preference separation | Product hypothesis | none | not learning | indirect | before training | both | KEEP |
...
- Последовательность включает input и output в один день.
- Translation правильно tied to native language, not profile language.
- Productive SRS уже есть в коде.
- Output artifacts и feedback уже концептуально присутствуют.

### Чего не хватает перед output

Перед “say your own” многим ученикам нужны:

1. 2-4 natural models;
2. highlighted target chunk;
3. meaning cue;
4. substitution slot;
...
### Чего не хватает после output

После output системе нужны:
...
## 4. Аудит механик повторения

Current SRS technically strong because it separates recognition and production. Methodologically, it should expose item state:
...
Не report "learned words", unless the label says exactly which state is meant.

## 5. Аудит AI interaction

AI полезен только когда он coach, not a pleasant chatty fog.
