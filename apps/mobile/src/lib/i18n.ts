...
const ru = {
  ...
  soundsX: {
    ...
    steps: {
      listen: { label: "1 · Слушай", hint: "Твоя роль: молчи и слушай, глазами по тексту. Полный темп носителя." },
      ultra: { label: "2 · Сверхмедленно", hint: "Твоя роль: повторяй ЗА каждым словом. Следи за губами и языком." },
      slow: { label: "3 · Вместе", hint: "Твоя роль: говори ВМЕСТЕ с голосом, темп замедлен. Сбилась — подхвати со следующего слова." },
      native: { label: "4 · Носитель", hint: "Звук тот же, что в шаге 1 — но теперь ГОВОРИШЬ ты, синхронно, в полный темп. Меняется роль, не запись." },
...
const en: Dict = {
  ...
  soundsX: {
    ...
    phraseOf: (i: number, n: number) => `${i} / ${n}`,
    steps: {
      listen: { label: "1 · Listen", hint: "Your role: stay silent and listen, eyes on the text. Full native speed." },
      ultra: { label: "2 · Ultra-slow", hint: "Your role: repeat AFTER each word. Watch your lips and tongue." },
      slow: { label: "3 · Together", hint: "Your role: speak ALONG with the voice, slowed down. Slipped? Rejoin at the next word." },
      native: { label: "4 · Native", hint: "Same audio as step 1 — but now YOU speak, in sync, at full speed. The role changes, not the recording." },
    } as Record<string, { label: string; hint: string }>,
    auto: "Autoflow",
    stop: "Stop",
