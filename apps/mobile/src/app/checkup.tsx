      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1, padding: 20, gap: 14, justifyContent: "center" }}>
        {stage === "intro" && (
          <View style={{ gap: 12 }}>
            ...
            <View style={{ backgroundColor: c.surface, borderRadius: radius.card, borderWidth: 1, borderColor: c.line, padding: 20, gap: 8 }}>
              ...
              <Text style={{ fontFamily: "GolosText_800ExtraBold", fontSize: 19, lineHeight: 27, color: c.ink }}>
                {prompt.en}
              </Text>
              <Text style={{ fontFamily: "GolosText_400Regular", fontSize: 13.5, lineHeight: 19, color: c.muted }}>
                {prompt.ru}
              </Text>
            </View>
            <Pressable
            ...
            >
              <Text style={{ fontFamily: "GolosText_700Bold", fontSize: 15, color: c.onBrand }}>{k.start}</Text>
            </Pressable>
          </View>
        )}
        ...
        {stage === "record" && (
          <View style={{ alignItems: "center", gap: 16 }}>
            <Text style={{ fontFamily: "GolosText_700Bold", fontSize: 17, lineHeight: 25, color: c.ink, textAlign: "center" }}>
              {prompt.en}
            </Text>
            <Text style={{ fontFamily: "GolosText_400Regular", fontSize: 13, color: c.muted, textAlign: "center", maxWidth: 300 }}>
            ...
            {!rec.recording && (
              <Pressable onPress={() => setStage("rate")} accessibilityRole="button" hitSlop={8}>
                <Text style={{ fontFamily: "GolosText_600SemiBold", fontSize: 13, color: c.muted }}>
                  {k.skipRecord}
                </Text>
              </Pressable>
            )}
        ...
        {stage === "rate" && (
          <View style={{ gap: 14 }}>
            <Text style={{ fontFamily: "GolosText_800ExtraBold", fontSize: 20, color: c.ink }}>{k.rateTitle}</Text>
            <Text style={{ fontFamily: "GolosText_400Regular", fontSize: 13, lineHeight: 19, color: c.muted }}>
              {k.rateNote}
            </Text>
            {rubric.map((r) => (
              <View key={r.key} style={{ gap: 8 }}>
                <Text style={{ fontFamily: "GolosText_600SemiBold", fontSize: 14, color: c.ink }}>{r.label}</Text>
                <View style={{ flexDirection: "row", gap: 8 }}>
                  {SCALE.map((n) => (
                    ...
                    >
                      <Text style={{ fontFamily: "GolosText_700Bold", fontSize: 15, color: scores[r.key] >= n ? c.brand : c.muted }}>
                        {n}
                      </Text>
                    </Pressable>
                  ))}
            ...
            >
              <Text style={{ fontFamily: "GolosText_700Bold", fontSize: 15, color: canFinish ? c.onBrand : c.muted }}>
                {k.finish}
              </Text>
            </Pressable>
          </View>
