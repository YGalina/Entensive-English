  ...
  if (state.block === "core" && state.corePhase === "produce") {
    return (
      <Shell state={state} title="Сессия дня" phase={4} onBack={returnToPlan}>
        ...
        <PrimaryButton label="Готово" disabled={!productionDraft.trim()} onPress={() => { const next = accept(submitStage0Day1Production(productionDraft)); if (!next) { setSaveMessage("Не сохранилось. Всё, что ты написала, — на экране. Попробуй ещё раз."); return; } setSaveMessage(null); }} />
      </Shell>
    );
  }
