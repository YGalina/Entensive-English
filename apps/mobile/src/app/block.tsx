  ...
  if (!resolved) {
    return (
      <View style={{ flex: 1, backgroundColor: c.bg, paddingTop: insets.top + 40, alignItems: "center" }}>
        <Text style={{ fontFamily: "GolosText_400Regular", fontSize: 14, color: c.muted }}>{b.notFound}</Text>
        <Pressable onPress={() => router.back()} style={{ marginTop: 16 }}>
          <Text style={{ fontFamily: "GolosText_700Bold", fontSize: 15, color: c.brand }}>{b.back}</Text>
        </Pressable>
      </View>
    );
  }
  ...
  return (
    <View style={{ flex: 1, backgroundColor: c.bg, paddingTop: insets.top + 10 }}>
      ...
      <View style={{ flexDirection: "row", alignItems: "center", paddingHorizontal: 20, gap: 10 }}>
        <View style={{ flex: 1 }}>
          <Text style={{ fontFamily: "GolosText_800ExtraBold", fontSize: 19, color: c.ink }} numberOfLines={1}>
            {script.title}
          </Text>
          <Text style={{ fontFamily: "GolosText_400Regular", fontSize: 11.5, color: c.muted }} numberOfLines={1}>
            {script.author}
          </Text>
        </View>
        <Pressable
      ...
      <ScrollView contentContainerStyle={{ padding: 20, gap: 14, paddingBottom: 40 }}>
        ...
        {phase === "warmup" && (
          <>
            ...
            <Text style={{ fontFamily: "GolosText_800ExtraBold", fontSize: 20, lineHeight: 27, color: c.ink }}>
            ...
            {block.chunks.map((ch, i) => (
              ...
              >
                <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
                  ...
                  <Text style={{ flex: 1, fontFamily: "GolosText_700Bold", fontSize: 15, color: c.ink }}>{ch.en}</Text>
                </View>
                <Text style={{ fontFamily: "GolosText_400Regular", fontSize: 13, color: c.muted, marginLeft: 22 }}>{ch.ru}</Text>
              </Pressable>
            ))}
            ...
            {script.lines.map((l, i) => (
              ...
              >
                <View style={{ flexDirection: "row", gap: 8 }}>
                  ...
                  <Text style={{ flex: 1, fontFamily: "GolosText_600SemiBold", fontSize: 14.5, lineHeight: 21, color: c.ink }}>{l.en}</Text>
                </View>
                {!!l.ru && (
                  <Text style={{ fontFamily: "GolosText_400Regular", fontSize: 12.5, color: c.muted, marginLeft: 23 }}>{l.ru}</Text>
                )}
              </Pressable>
            ))}
        ...
        {phase === "explain" && (
          <>
            ...
            <Text style={{ fontFamily: "GolosText_700Bold", fontSize: 16, lineHeight: 24, color: c.ink }}>
            ...
            <TextInput
              ...
              multiline
              style={{ minHeight: 96, borderRadius: 12, borderWidth: 1, borderColor: c.line, backgroundColor: c.surface, padding: 12, fontFamily: "GolosText_400Regular", fontSize: 15, color: c.ink, textAlignVertical: "top" }}
            />
          ...
          <View style={{ alignItems: "center", gap: 10, paddingTop: 20 }}>
            ...
            {hints !== null && hints.length > 0 && (
              <View style={{ backgroundColor: c.brandSoft, borderRadius: 12, padding: 12, gap: 4, width: "100%" }}>
                <Text style={{ fontFamily: "GolosText_600SemiBold", fontSize: 11, color: c.brandInk }}>{b.hintTitle}</Text>
                {hints.map((h) => (
            ...
            >
              <Text style={{ fontFamily: "GolosText_700Bold", fontSize: 15, color: c.onBrand }}>{b.home}</Text>
            </Pressable>
          </View>
        )}
      </ScrollView>
...
function PhaseButton({
  ...
  return (
    ...
    >
      <Text style={{ fontFamily: "GolosText_700Bold", fontSize: 15, color: disabled ? c.muted : c.onBrand }}>
        {label}
      </Text>
    </Pressable>
  );
