...
**Физический инвентарь файлов (исправлено):**
- **24 обязательных записанных исходных файла**;
- **1 опциональный записанный исходный файл: A4**;
- **9 обязательных производных нарезок: D1–D6 и SH1–SH3** (не записываются отдельно — монтаж из мастеров, §7).

Прежняя формулировка «12 обязательных мастеров + 1 опциональный» считала ГРУППЫ записей, а не физические файлы (A5 = 4 файла, A6 = 4, P1 = 9, P2 = 2), и снята как неточная.

### Сверочная таблица

| Группа записи | Физических исходных файлов | Обяз./опц. | Производные выходы |
|---|---|---|---|
| A1 (T1, обычный) | 1 | обязателен | SH1, SH2, SH3 |
| A1-f (T1, беглый) | 1 | обязателен | D1, D2, D3 |
| A2 (T2, обычный) | 1 | обязателен | — |
| A2-f (T2, беглый) | 1 | обязателен | D4, D5, D6 |
| A3 (T3, обычный) | 1 | обязателен | — |
| A4 (T4, чтение) | 1 | **опционален** | — |
| A5 (T5, реплики r1–r4) | 4 | обязательны | — |
| A6 (сценарии s1, s2, s2r, s3) | 4 | обязательны | — |
| P1 (пары ударения ×6 + фразы ×3) | 9 | обязательны | — |
| P2 (estimate: глагол, сущ.) | 2 | обязательны | — |
| **Итого исходных** | **24 обязательных + 1 опциональный** | | **9 производных нарезок** |
...
- [ ] **Все 24 обязательных исходных файла присутствуют** (сверка по сверочной таблице §2)
- [ ] **Опциональный A4 явно помечен** как записанный или как пропущенный (одно из двух, без умолчания)
- [ ] **Все 9 производных нарезок присутствуют** (D1–D6, SH1–SH3, по карте §7)
- [ ] **У каждого обязательного исходного файла есть утверждённый RAW WAV и оба экспортных формата доставки** (WAV 44.1/16 + MP3, §9)
