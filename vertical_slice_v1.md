...
**22 объекта + 1 конструкция + 6 контрольных (holdout) единиц** — контрольные подобраны парами к тренируемым (та же структура/регистр/частотный слой), участвуют ТОЛЬКО в претесте и дне 14, в уроках не появляются никогда (см. §7).
...
**Recovery-маршрут (политика — явно):** пропуск ≥2 календарных дней → хаб предлагает «Вернуться с 5 минут»: короткая сессия-возврат (до 8 due-единиц + 1 производство, без нового материала). Возврат — **рекомендованный, но не обязательный** путь после паузы: полная сессия остаётся доступной (вторично, ghost-кнопкой). Возврат **не тратит программу**: `sessionsCompleted` и дни введения не двигаются, ведётся отдельный счётчик `recoveriesCompleted` (тип `recovery` в логе). Последовательность введения идёт по счёту учебных сессий, не по календарю; пропуски не наказываются и нигде не называются «разрывом».

---

## 7. Измерение (пункты 4–5)

**Метка `structural generalization` не используется** — протокол её не поддерживает. Вместо этого — **три СТРОГО раздельных класса** (в экспорте разведены и никогда не складываются):

| Когда | Что | Класс |
|---|---|---|
| День 0 | Претест: письменное извлечение всех **28** единиц (22 тренируемые + 6 контрольных, перемешаны; смысл→форма, без подсказок; «не помню» — легальная кнопка) | базовая линия |
| ≥ день 14 (календарных от претеста) | Тот же тест, те же 28 единиц, тот же порядок | **(1) retention тренированных** (дельта к претесту по 22) · **(2) holdout-контроль** (дельта по 6 нетренированным — контроль test–retest и фоновой экспозиции; НЕ «генерализация») |
| Сразу после | Новый промпт: *«Tell me about a problem you solved at work recently»* — письменный ответ + опциональный голос | **(3) употребление тренированных в новом контексте** — по opportunity-модели: знаменатель = 8 целевых единиц, которым промпт даёт реальную возможность; `used / missed / off-target` — явные состояния |

`HoldoutAssignment` (ученик × набор × эксперимент) фиксируется на претесте и пишется в лог. Отложенный тест открывается по календарю (≥14 дней от претеста), даже если сессий меньше 14 — интервал удержания важнее полноты программы. Real-world transfer — не измеряется, нечем.

**Валидация конструкции — настоящая, не «слово been в тексте»:** present perfect continuous = форма have/has (+нар.) + been + V-ing с фильтром не-глаголов; past simple = прошедшая форма глагола И явный маркер времени. Сигналы пишутся в каждое production-событие; это по-прежнему поверхностный чек формы — смысловую уместность смотрит человек по экспорту.
...
- «11 голосовых записей — твои, приватные, не оцениваются»
- «Форма have/has been + -ing вышла в твоих письменных ответах: 6» (валидированная конструкция; факт письменного текста)
- «Возвратов после паузы: 2 (программу не тратят)»
- «По 3 единицам данных пока мало» (честное `недостаточно данных`)
Запрещено: «способности», «что могу», «выучено», «стало твоим», уровни, проценты курса, стрики, «молодец» — и любые формулировки, выдающие письменное за устное («сказала», «речь») в метриках.
...
Opportunity-модель (знаменатель = 8): solve a problem · figure out · It turned out that… · The thing is… · sort out · come up with · make a decision · challenging.

### Контрольный (holdout) набор — только претест и день 14, в уроках никогда
| EN | RU | Пара к |
|---|---|---|
| set a goal | поставить цель | make a decision |
| make progress | продвинуться, сделать прогресс | solve a problem |
| hold a meeting | провести встречу | run a project |
| carry out | выполнить (план, задачу) | figure out |
| bring up | поднять (тему, вопрос) | come up with |
| turn down | отклонить (предложение) | put off |
...
## 12. Вырезано (замок)
Веб целиком · Библиотека/Словарь/Люди/Профиль-переделки · вечерний круг · YouTube/импорт · лендинг/оплата/авторизация · онбординг · уведомления · движок узких мест · стражи и ставки · T5/сжатие · живая практика · AI-runtime · ASR · стрики · миграции общей БД.
